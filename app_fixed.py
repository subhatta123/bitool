...
def show_dashboard_page():
    ...
    # User existence is checked at login, no need for: if st.session_state.logged_in_user not in USERS_DATA:

    st.sidebar.subheader("Manage Dashboards")
    # dashboard_name_tuples is now list of (raw_name, display_name, owner_username)
    ...
    if current_selected_raw_name:
        for raw, display, owner in dashboard_name_tuples:
            if raw == current_selected_raw_name:
                ...
                current_dashboard_owner = owner # Capture owner of the currently selected dashboard
                break
    ...
    if dashboard_display_options:
        ...
        if selected_raw_name_from_select and selected_raw_name_from_select != st.session_state.current_dashboard_name:
            ...
            # When loading, username is the logged_in_user, dashboard_name is selected_raw_name_from_select
            st.session_state.dashboard_items = load_user_dashboard(st.session_state.logged_in_user, selected_raw_name_from_select)
            current_dashboard_owner = selected_owner_from_select # Update owner for actions section
            st.rerun()
        elif not st.session_state.current_dashboard_name and selected_raw_name_from_select: # If initially no dash selected, but one is now selected
            st.session_state.current_dashboard_name = selected_raw_name_from_select
            st.session_state.dashboard_items = load_user_dashboard(st.session_state.logged_in_user, selected_raw_name_from_select)
    ...
    if st.session_state.current_dashboard_name:
        # current_dashboard_owner should be set if current_dashboard_name is set (from selection or creation)
        # Re-fetch owner if not set (e.g. direct page load with session state)
            ...
            if is_owner:
                ...
                if st.button("Rename Dashboard", key="rename_selected_dashboard_button"):
                    if new_dashboard_rename_input and new_dashboard_rename_input.strip():
                        ...
                        # Check if current user already owns a dash with the new name
